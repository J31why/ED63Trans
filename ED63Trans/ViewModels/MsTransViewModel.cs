...
public partial class MsTransViewModel: ViewModelBase
{
    ...
    {
        _msDatFiles =Directory.EnumerateFiles("E:\\SteamLibrary\\steamapps\\common\\Trails in the Sky the 3rd\\ED6_DT30 - 副本","ms*._dt").ToList();
        // var bytes = SoraFont.JisEncoding.GetBytes("end this");
        //
        // foreach (var file in _msDatFiles)
        // {
        //     var data = File.ReadAllBytes(file);
        //     if (IndexOf(data, 0, bytes).Any())
        //     {
        //         
        //     }
        // }
    ...
    public  bool ValidateBytes(byte[] bytes)
    {
        ...
        if (bytes is [0xF4, 1] or [0xA2, 1] or [0xE8,3] or [0x8D,3]  or [0x4e,0x1])
            return false;
        return true;
    }
    public static bool Validate(string text)
    {
        ...
        return !Regex.IsMatch(text, 
            @"[\u0001-\u001f]|gCTI0|€||P\?|P・|ﾃ|龝|mpa\\|Craft\\|\ue7e0|monster\\|magic\\|craft\\|scraft\\|battle\\|map\\"
            );
    }
